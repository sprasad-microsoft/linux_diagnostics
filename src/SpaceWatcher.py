...
class SpaceWatcher:
    def __init__(self, controller):
        ...
        self.aod_output_dir = cfg.get(
            "aod_output_dir", "/var/log/aod"
        )  # Default to /var/log/aod if not set
        print(
            f"[SpaceWatcher] Initializing with max_age={self.max_age} days, max_size={self.max_size} MB, interval={self.cleanup_interval} sec"
        )
    ...
    def _check_space(self) -> bool:
        ...
        if total_size > self.max_size * 1024 * 1024:  # Convert MB to bytes
            print(
                f"[SpaceWatcher] Total log size {total_size / (1024 * 1024):.2f} MB exceeds max {self.max_size} MB"
            )
            return True
        return False
    ...
    def cleanup_by_age(self) -> None:
        ...
        to_delete_batches = [
            d
            for d in self.batches_root.iterdir()
    ...
    def cleanup_by_size(self) -> None:
        ...
        print(
            f"[SpaceWatcher] Total size of batches: {total_size / (1024 * 1024):.2f} MB, max allowed: {self.max_size} MB"
        )

        deleted = 0
        # first check if deleting completed batches suffices
        for batch in sorted_batches:
            if total_size <= max_bytes * 0.5:
                break
